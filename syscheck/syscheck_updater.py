...
from syscheck.helpers import validation_helpers, info_helpers
...
def analyse_syscheck_data(
    ...
    region = info_helpers.get_console_region(data)
    hbc_version = info_helpers.get_hbc_version(data)
    system_menu_version = info_helpers.get_system_menu_version(data)
    firmware = info_helpers.get_firmware(system_menu_version) if system_menu_version else None
    console_type = info_helpers.get_console_type(data)
    ...
    if not console_type or not validation_helpers.validate_console_type(console_type):
        raise SyscheckError('The CSV file does not contain a valid console type')

    region_short_code = info_helpers.get_region_short_code(region)
    ...
    is_bootmii_installed = validation_helpers.check_if_bootmii_installed(data)
    is_priiloader_installed = validation_helpers.check_if_priiloader_installed(data)
    is_hbc_outdated = validation_helpers.check_if_hbc_is_outdated(hbc_version, console_type) if hbc_version else False
    outdated_d2xcios = validation_helpers.check_d2x_cios(data, console_type)
    missing_ios = validation_helpers.check_for_missing_ios(data, region, console_type) if activeIOS else []
    needs_extra_protection = validation_helpers.check_extra_protection(data) if extraProtection else []
    ...
    latest_firmware_version = info_helpers.get_latest_sm_version(firmware)
